...
class Explosion(pygame.sprite.Sprite): #定義(繼承)

    def __init__(self, center, size): #建構元
        pygame.sprite.Sprite.__init__(self)
        self.size = size
    ...
    def update(self):
        ...
        if now - self.last_update > self.frame_rate:
            ...
            self.frame += 1
            if self.frame == len(expl_anim[self.size]): #最後一張
                self.kill()
            else:
...
class Power(pygame.sprite.Sprite): #定義(繼承)
    def __init__(self, center): #建構元
        pygame.sprite.Sprite.__init__(self)
...
while running:
    ...
    for hit in hits:
        ...
        if random.random() > 0.9:
            pow = Power(hit.rect.center) #寶物物件實作
            all_sprites.add(pow)
            powers.add(pow)
    ...
    for hit in hits:
        ...
        expl = Explosion(hit.rect.center, 'sm') #小爆炸
        all_sprites.add(expl)
        if player.health <= 0:
